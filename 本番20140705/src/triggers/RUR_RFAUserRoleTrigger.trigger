...
trigger RUR_RFAUserRoleTrigger on RFAUserRole__c (
    ...
    Email_Handler_RFA instEmail = new Email_Handler_RFA();
    ...
    RFA_TriggerHandler_RFAUserRole instTriggerHandler = new RFA_TriggerHandler_RFAUserRole();
            ...
            if ( app.RecordTypeId == approverRecordTypeMap.get(RFAConstants.RTLabel_RFAUserRole_Approver).Id 
            	...
            	&&
                  ( app.RFAStage__c == RFAConstants.RFA_STATUS_CLOSED ||
                	...
                	//added by Jia Hu on 2014/02/14  need to update Label message
                  ) 
               )
            {
            	//??? need new Custom Label for RFA_STATUS_CIRCULATION  
    ...
    //added by Jia Hu on 2014/05/23
    if(trigger.IsInsert && trigger.IsAfter) {
    	instTriggerHandler.processRFAUserRolesInAfterInsertTrigger(trigger.new);
    }
    ...
    {		          	
    	//instTriggerHandler.checkApproverOrderChanges(trigger.new, trigger.oldMap);
    	
    	instTriggerHandler.processRFAUserRolesInAfterUpdateTrigger(trigger.new, trigger.oldMap);
    ...
    {						        
    	//New Handler added by Jia Hu on 2014/05/22
    	instTriggerHandler.processRFAUserRolesInAfterDeleteTrigger(trigger.old);
